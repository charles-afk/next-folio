...
export default function Hamburger({
  ...
  const pathname = usePathname();
	...
	const aboutMeClicked = (): void => {
		setSelectedIndex(1);
		setIsOpen(false);
	};

	const hireMeClicked = (): void => {
		setSelectedIndex(2);
		setIsOpen(false);
	};

  	const portfolioClicked = (): void => {
		setSelectedIndex(3);
		setIsOpen(false);
	};
  ...
  return (
        <Fragment>
            <div className="relative">
                <div className={`z-50 ${pathname === "/" ? "top-[0px]" : "top-[0px]"} cursor-pointer fixed w-14 h-14 lg:w-24 lg:h-24 bg-[#48AFDE] top-0 flex justify-center items-center rounded-br-3xl `}>
                    <div onClick={() => setIsOpen(false)} className="relative w-7 lg:w-10 h-7 lg:h-10 flex justify-center items-center">
                    {isOpen ? (
						<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-12 h-12 text-white cursor-pointer">
            ...
            <header className={`fixed w-full transition-all duration-500 z-40 ${ pathname === "/" ? "top-0" : "top-[0px]" }`}>
              				...
              				<ul className="flex flex-col">
								<li className="group my-4 xl:my-4 2xl:my-6 relative">
									<div className="inline-block relative">
										<a target="blank" href="/#home" onClick={homeClicked}>
											Home
										</a>
										{selectedIndex === 0 ? (
								...
								<li className="group my-4 xl:my-4 2xl:my-6 relative">
									<div className="inline-block relative">
										<a target="blank" href="/#about-me-component" onClick={aboutMeClicked} >
											About Me
										</a>
										{selectedIndex === 1 ? (
											<div className="absolute top-2  -left-2 w-full h-full transform-gpu transition-all duration-300 -rotate-6  opacity-100  group-hover:-rotate-6 group-hover:opacity-100 bg-[#48AFDE] rounded-xl -z-10"></div>
								...
								<li className="group my-4 xl:my-4 2xl:my-6 relative">
									<div className="inline-block relative">
										<Link target="blank" href="/contact-me" prefetch={false} onClick={hireMeClicked}>
											Hire Me
										</Link>
										{selectedIndex === 2 ? (
											<div className="absolute top-2  -left-2 w-full h-full transform-gpu transition-all duration-300 -rotate-6  opacity-100  group-hover:-rotate-6 group-hover:opacity-100 bg-[#48AFDE] rounded-xl -z-10"></div>
                ...
                <li className="group my-4 xl:my-4 2xl:my-6 relative">
									<div className="inline-block relative">
										<Link target="blank" href="/portfolio" prefetch={false} onClick={portfolioClicked}>
											Portfolio
										</Link>
										{selectedIndex === 3 ? (
											<div className="absolute top-2  -left-2 w-full h-full transform-gpu transition-all duration-300 -rotate-6  opacity-100  group-hover:-rotate-6 group-hover:opacity-100 bg-[#48AFDE] rounded-xl -z-10"></div>
            			...
            			<section className="mt-14 sm:mt-12 lg:mt-14 text-center relative sm:absolute lg:relative sm:right-0 lg:right-0 h-auto sm:h-full lg:h-auto">
							<h3 className="block sm:hidden lg:block font-bold text-[#48AFDE]  text-2xl uppercase mb-5">
              				...
              				<div className="flex flex-row sm:flex-col  lg:flex-row justify-center">
								<a target="blank" href="https://github.com/charles-afk" className="text-gray-300 hover:text-white transition-colors duration-3000">
									<svg stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 496 512" height="1em" width="1em"
									className="w-6 h-6 sm:w-8 sm:h-8 mb-0 sm:mb-5 lg:mb-0 mx-3 sm:mx-0 lg:mx-3" xmlns="http://www.w3.org/2000/svg">
								...
								<a target="blank" href="https://www.linkedin.com/in/charles-agudelo-55756b1a3/" className="text-gray-300 hover:text-white transition-colors duration-3000">
